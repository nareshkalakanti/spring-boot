...
import org.springframework.boot.diagnostics.BeanCurrentlyInCreationFailureAnalyzerTests.CycleWithAutowiredFields.BeanThreeConfiguration;
import org.springframework.boot.diagnostics.BeanCurrentlyInCreationFailureAnalyzerTests.CycleWithAutowiredFields.BeanTwoConfiguration;
...
public class BeanCurrentlyInCreationFailureAnalyzerTests {
	...
	public void cyclicBeanMethods() {
		FailureAnalysis analysis = this.analyzer
				.analyze(createFailure(CyclicBeanMethodsConfiguration.class));
	...
	public void cycleWithAutowiredFields() {
		FailureAnalysis analysis = this.analyzer
				.analyze(createFailure(CycleWithAutowiredFields.class));
		assertThat(analysis).isNotNull();
		assertThat(analysis.getDescription()).startsWith(
				"There is a circular dependency between 3 beans in the application context:");
		assertThat(analysis.getDescription())
				.contains("three defined in " + BeanThreeConfiguration.class);
		assertThat(analysis.getDescription())
				.contains("one defined in " + CycleWithAutowiredFields.class.getName());
		assertThat(analysis.getDescription())
				.contains(BeanTwoConfiguration.class.getName() + " (field private "
						+ BeanThree.class.getName());
	}
