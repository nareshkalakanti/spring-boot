...
|`premainClass`
|The agent class that contains a `premain` method that should be called when the JVM is
launched. Configures the `Premain-Class` attribute in the repackaged archive's manifest,
allowing the archive to be used as the `-javaagent` value when the JVM is launched. When
`premainClass` is specified, the manifest's `Can-Redefine-Classes` and
`Can-Retransform-Classes` attributes are both configured with a value of `true`.
